        ...
        return (
          <div className="relative w-full h-full flex items-center justify-center">
             ...
             <motion.div 
               ...
               className="relative z-10 w-16 h-16 bg-brand-navy border border-brand-teal/30 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(121,243,222,0.1)]"
             >
...
export const ChallengesSection: React.FC = () => (
  <section className="py-20 bg-brand-section transition-colors duration-300">
      ...
      >
        {capabilities.map((card, i) => (
          <motion.div
            key={i}
            variants={itemVariants}
            whileHover={{ y: -5, boxShadow: "0 10px 40px rgba(121,243,222,0.1)" }}
            className="group bg-brand-card border border-brand-border p-8 rounded-xl cursor-default transition-all duration-300"
          >
            <div className="w-16 h-16 flex items-center justify-center rounded-lg bg-brand-navy/30 mb-6 group-hover:bg-brand-teal/10 transition-colors duration-300">
               <card.icon className="w-8 h-8 text-brand-teal group-hover:scale-110 group-hover:drop-shadow-[0_0_8px_rgba(121,243,222,0.5)] transition-all duration-300" />
            </div>
          ...
          <div className="lg:w-1/3 flex lg:flex-col gap-4 overflow-x-auto lg:overflow-visible pb-4 lg:pb-0">
            {processSteps.map((step) => (
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className={`flex items-center gap-4 p-4 rounded-lg border text-left transition-all min-w-[260px] lg:min-w-0 ${
                  activeStep === step.id 
                    ? 'bg-brand-card border-brand-teal shadow-[0_0_15px_rgba(121,243,222,0.1)]' 
