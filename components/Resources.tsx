...
interface InsightsSectionProps {
  onNavigate?: (page: 'home' | 'about' | 'services' | 'contact' | 'faq' | 'insights') => void;
}
    ...
    <div className="container mx-auto px-6">
      ...
      >
        ...
        <form className="flex w-full md:w-auto gap-4" onSubmit={(e) => e.preventDefault()}>
          ...
          />
          <button className="bg-brand-teal text-brand-navy font-bold px-6 py-3 rounded hover:bg-brand-white transition-colors">
            Subscribe
          </button>
        </form>
      </motion.div>
          ...
          {faqs.map((faq, index) => (
            <motion.div 
              ...
              className={`border transition-all duration-300 rounded-lg overflow-hidden ${
                openIndex === index 
                  ? 'border-brand-teal bg-brand-navy shadow-[0_0_20px_rgba(121,243,222,0.05)]' 
